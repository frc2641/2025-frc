...
// import frc.team2641.robot2025.Constants.DriveTrainConstants;
 ...
 */
public class OpponentRobotSimulation {
    ...
    public OpponentRobotSimulation(int id, CompetitionFieldSimulation simulation) {
        ...
        behaviorChooser.setDefaultOption(
                "Auto Cycle",
                this.getAutoCyleRepeadtelyCommand(
                        switch (robotID) {
    ...
    public void updateSimulationSubTick(int iterationNum, double subPeriodSeconds) {
        super.simulateChassisBehaviorWithRobotRelativeSpeeds(speedSetPoint);
    }

    private static final PathConstraints constraints =
            new PathConstraints(3.5, 8, Math.toRadians(180), Math.toRadians(360));

    public Command getAutoCyleRepeadtelyCommand(Command toRunAtEndOfCycle) {
        final PathPlannerPath
                cycleForwardPath =
                        MaplePathPlannerLoader.fromPathFile("other robot cycle path " + robotID, constraints),
                cycleBackwardPath =
